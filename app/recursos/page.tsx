...
import { ArrowLeft, Download, ExternalLink, FileText, FolderOpen, House, Library, Menu, Moon, Sun, X } from 'lucide-react';
    ...
    <section className="conversation resources-conversation">
      ...
      <div className="resources-main">
        ...
        <section id="guias" className="resources-section" aria-labelledby="resources-title"><div className="resources-section-head"><div><span>01</span><h2 id="resources-title">Biblioteca</h2></div><p>{resources.length} recursos disponibles</p></div><div className="resources-grid">
          {resources.map(resource => { const Icon = resource.icon; const ActionIcon = resource.download ? Download : ExternalLink; return <a key={resource.title} className="resource-card" href={resource.href} download={resource.download} target={resource.download ? undefined : '_blank'} rel={resource.download ? undefined : 'noreferrer'}><div className="resource-card-top"><span className="resource-icon"><Icon size={19}/></span><span className="resource-type">{resource.type}</span><span className="resource-action"><ActionIcon size={16}/></span></div><div><h3>{resource.title}</h3><p>{resource.description}</p></div><span className="resource-meta">{resource.meta}</span></a>; })}
