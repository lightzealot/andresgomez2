...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const delay = reducedMotion ? 90 : 430;
    const timer = window.setInterval(() => {
      setBootStep(current => {
        if (current >= bootSteps.length - 1) {
          window.clearInterval(timer);
          window.setTimeout(() => setBooting(false), reducedMotion ? 80 : 520);
          return current;
        }
        return current + 1;
      });
  ...
  <main className="portfolio-shell">
    ...
    <aside className={`sidebar ${mobileOpen ? 'is-open' : ''}`}>
      ...
      <div className="sidebar-footer"><div className="status-dot"/><div><strong>Andrés Gómez</strong><span>Aprende IA sin complicarte</span></div></div>
    </aside>
    ...
    <section className="conversation">
      ...
      <div className="thread">
        <section id="inicio" className="welcome section-block"><div className="assistant-avatar">AG</div><div><p className="eyebrow">Andrés Gómez / creador digital</p><h1>Aprende inteligencia artificial sin complicarte.</h1><p className="lead">Prompts, imágenes, automatización y agentes explicados desde cero para que pases de consumir contenido a crear con inteligencia artificial.</p><div className="profile-topics"><span>🧠 Prompts</span><span>🎨 Imágenes</span><span>⚙ Automatización</span><span>🤖 Agentes</span></div><div className="system-status"><span>RUTA ACTIVA</span><strong>De cero a crear con IA</strong></div><div className="quick-actions"><button onClick={() => goTo('proyectos')}>Empezar desde cero <ArrowUp size={15}/></button><button onClick={() => goTo('sobre-mi')}>Conoce a Andrés</button></div></div></section>
