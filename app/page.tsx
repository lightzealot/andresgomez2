...
export default function Home() {
  ...
  <main className={`portfolio-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    <button className="mobile-menu" onClick={() => { setSidebarCollapsed(false); setMobileOpen(true); }} aria-label="Abrir menú"><Menu size={20}/></button>
      ...
      <div className="recent"><p className="nav-label">Labs | Playground</p><button onClick={() => goTo('labs')}><Folder size={15}/> IA desde cero</button><button onClick={() => goTo('labs')}><Folder size={15}/> Prompts que funcionan</button><button onClick={() => goTo('labs')}><Folder size={15}/> Primera automatización</button><button onClick={() => goTo('labs')}><Folder size={15}/> Herramientas IA gratis</button></div>
