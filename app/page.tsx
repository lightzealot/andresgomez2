'use client';
import { ArrowUp, BriefcaseBusiness, ExternalLink, Folder, Library, Mail, Menu, MessageSquare, Moon, PanelLeftClose, Sun, UserRound, X } from 'lucide-react';
  ...
  <main className={`portfolio-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    <button className="mobile-menu" onClick={() => { setSidebarCollapsed(false); setMobileOpen(true); }} aria-label="Abrir menú"><Menu size={20}/></button>
      ...
      <div className="sidebar-top">
        <button className="brand" onClick={() => { setSidebarCollapsed(false); goTo('inicio'); }} aria-label="Abrir barra lateral"><span className="brand-mark"><img src="/andresgomezos-logo.png" alt=""/></span><span className="brand-name">AndresGomezOS</span><span className="sidebar-tooltip" role="tooltip">Abrir barra lateral</span></button>
      ...
      <nav aria-label="Navegación principal"><p className="nav-label">Explorar</p>
        ...
        <Link className="nav-item" href="/recursos"><Library size={17}/><span>Recursos</span></Link>
      </nav>
